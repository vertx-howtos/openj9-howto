...
* We will check the options for improving startup time with OpenJ9.
...
Let us simulate 2000 users with a 50 new users per second hatch rate.
This gives us about 600 requests per second.

=== Measuring RSS

The https://quarkus.io/guides/performance-measure[Quarkus team has a good guide on measuring RSS].

By letting Locust operate with 2000 users for a few minutes, I get a RSS of about *96 MB*.

With OpenJDK 11 and no tuning, I get a RSS of about *247 MB*.

Let us use OpenJDK 11 again but now with some limits:

----
$ java -Xmx32m -Xss256k -jar build/libs/openj9-howto-all.jar
----

RSS usage goes to *124 MB* on my machine.

Now let us go back to OpenJ9 with a similar configuration:

----
$ java -Xmx32m -Xss256k -Xshareclasses -Xshareclasses:name=sum -Xshareclasses:cacheDir=_cache -jar build/libs/openj9-howto-all.jar
----

RSS usage is about *73 MB* on my machine.
